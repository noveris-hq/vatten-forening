<div class="space-y-3">
    @foreach ($groupedDocuments as $year => $docs)
        <h3 class="mt-6 mb-2 text-lg font-semibold">{{ $year }}</h3>
        @foreach ($docs as $doc)
            <div
                ...
                <div class="flex items-center gap-4">
                    ...
                    @if (auth()->user()?->is_admin)
                        <form action="{{ route('admin.document.destroy', $doc) }}" method="POST" class="inline"
                            ...
                            @method('DELETE')
                            <button type="submit"
                                class="text-sm text-red-600 hover:text-red-800 hover:underline hover:cursor-pointer bg-red-100 px-3 py-1.5 rounded-md">
                                Ta bort
                            </button>
                        </form>
                    @endif
